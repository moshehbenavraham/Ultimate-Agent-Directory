...
class AgentEntry(BaseModel):
    ...
    url: HttpUrl = Field(
        description="Primary URL (GitHub, website, or documentation)"
    )
    ...
    category: str = Field(
        description="Primary category (must match category ID)"
    )

    # ===== CLASSIFICATION METADATA =====
    subcategory: Optional[str] = Field(
        default=None,
        description="Optional subcategory (e.g., 'core-frameworks')"
    )
    ...
    platform: Optional[List[str]] = Field(
        default=None,
        description="Languages/platforms (e.g., ['Python', 'TypeScript'])"
    )
    ...
    github_stars: Optional[int] = Field(
        default=None,
        description="Auto-updated by GitHub Action"
    )

    last_updated: Optional[date] = Field(
        default=None,
        description="Last commit date (auto-updated)"
    )

    is_archived: bool = Field(
        default=False,
        description="Is GitHub repo archived?"
    )
    ...
    verified: bool = Field(
        default=False,
        description="Link checked, metadata complete?"
    )

    # ===== METADATA TRACKING =====
    added_date: Optional[date] = Field(
        default=None,
        description="When added to directory"
    )
...
class DirectoryMetadata(BaseModel):
    ...
    maintainer_url: HttpUrl = "https://AIwithApex.com"

    last_generated: date = Field(
        default_factory=date.today,
        description="When README/website was last generated"
    )
    ...
    badges: dict = Field(
        ...
        description="Badge counts by type"
    )
